...
import jp.ryo.informationPump.server.db.results.webdatadbmanager.SelectEntriesBeforeADateResult;
import jp.crossfire.framework.database.ISelectResultListener;

public class WebDataDBManagerSelectEntriesBeforeADateListner implements ISelectResultListener {
	...
	public  WebDataDBManagerSelectEntriesBeforeADateListner() {
		Hashtable hash = null;
		hash = new Hashtable();
	...
	public void orMap() {
		...
		while(it.hasNext()){
			...
			SelectEntriesBeforeADateResult data = (SelectEntriesBeforeADateResult)it.next();
	...
	public void getResult(ResultSet result, ResultSetMetaData meta) throws Exception {
		...
		SelectEntriesBeforeADateResult data = new SelectEntriesBeforeADateResult();
