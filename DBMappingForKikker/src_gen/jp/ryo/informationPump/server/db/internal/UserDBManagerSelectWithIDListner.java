...
import jp.ryo.informationPump.server.db.results.userdbmanager.SelectWithIDResult;
import jp.crossfire.framework.database.ISelectResultListener;

public class UserDBManagerSelectWithIDListner implements ISelectResultListener {
	public List result = new LinkedList();
	public Hashtable tables = new Hashtable();
	public List userentryTmpCache = new LinkedList();
	public  UserDBManagerSelectWithIDListner() {
		Hashtable hash = null;
		hash = new Hashtable();
	...
	public void orMap() {
		...
		while(it.hasNext()){
			...
			SelectWithIDResult data = (SelectWithIDResult)it.next();
	...
	public void getResult(ResultSet result, ResultSetMetaData meta) throws Exception {
		...
		SelectWithIDResult data = new SelectWithIDResult();
