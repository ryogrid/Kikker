...
import jp.ryo.informationPump.server.db.results.webdatadbmanager.SelectWithAddressResult;
import jp.crossfire.framework.database.ISelectResultListener;

public class WebDataDBManagerSelectWithAddressListner implements ISelectResultListener {
	...
	public  WebDataDBManagerSelectWithAddressListner() {
		Hashtable hash = null;
		hash = new Hashtable();
	...
	public void orMap() {
		...
		while(it.hasNext()){
			...
			SelectWithAddressResult data = (SelectWithAddressResult)it.next();
	...
	public void getResult(ResultSet result, ResultSetMetaData meta) throws Exception {
		...
		SelectWithAddressResult data = new SelectWithAddressResult();
